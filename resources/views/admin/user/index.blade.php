...
@section('content')
@section('breadcrumb_title', trans('Manage Users'))
@section('breadcrumb_pagename', trans('User List'))
...
<script>
    $(document).ready(function() {
        ...
        $('#btnObjectClose').on('click', function(e) {
            e.preventDefault();
            $('#frmCrudObject').find('#showPhoto').attr('src',
                "{{ asset('public/images/avatar3.png') }}");
            $('#frmCrudObject').find('#btnObjectSave').removeClass('d-none');
            $('#frmCrudObject').find('#btnObjectUpdate').addClass('d-none');
            $('#crudObjectModal').find('.modal-title').html(
                '{{ trans('global.add') }} {{ trans('cruds.user.title_singular') }}');
